...
def compute_fscores(pred, gold, verbose=False):
    ...
    fscore1, _, _ = compute_macro_fscore(pred_filtered,
                                         gold_filtered,
                                         track=1,
                                         verbose=False)
    if verbose:
    ...
    fscore3, classwise_fscores, mcm = compute_macro_fscore(pred, gold, track=3, verbose=False)
    if verbose:
    ...
    scores = {"track1":fscore1, "track2":fscore2, "track3":fscore3}
    scores["classwise_fscores"] = classwise_fscores
    scores["confusion_matrix"] = conf
    scores["multilabel_confusion_matrix"] = mcm 
    return scores
...
def compute_macro_fscore(pred, gold, track=1, verbose=False):
    ...
    f1scores = np.array(f1scores, dtype=np.float)
    return macro_avg, f1scores, conf
